...
async def start_session(
    ...
    prompt = json.dumps({
        ...
        "instructions": (
            f"Generate exactly {target_q} evaluation questions PLUS 1 optional application-based bonus question. "
            "STRICT RULE: Every question (except the bonus) MUST be answerable from the lesson content above. "
            "Zero out-of-syllabus questions for the base set. "
            "Question mix for base questions: "
            "  Q1: recall — ask the student to explain or define something taught in the lesson. "
            "  Q2: conceptual/application — ask the student to apply or reason about what was taught. "
            "  Q3+ (if target >= 3): misconception check — present a common wrong assumption and ask if it is correct. "
            "For coding lessons: Q2 should be 'what does this code do' or 'what is the output'. "
            "For math/science: Q2 should apply a formula or explain a step. "
            "For humanities: Q2 should be cause-effect or ask about a key actor/decision. "
            "BONUS QUESTION: 1 application-based question that requires the student to USE the concept in a "
            "new scenario (slightly beyond the lesson — tests transfer, not recall). "
            "Mark the bonus question with 'is_bonus': true and 'is_base_question': false. "
            "The bonus question's score is tracked separately and does NOT count toward pass/fail. "
            "Return JSON only."
        ),
        "pace_context": (
            f"Pace is '{pace}'. "
